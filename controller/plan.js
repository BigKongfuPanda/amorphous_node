...
class Plan {
  ...
  // 化钢工序根据排产日期和材质来查询生产计划列表
  async queryDataByMelt(req, res, next) {
    const { castId, dateJson, ribbonTypeName } = req.query;
    try {
      if (!castId || !ribbonTypeName) {
        throw new Error("参数错错误");
      }
    } catch (err) {
      console.log(err.message, err);
      res.send({
    ...
    try {
      ...
      const dateList = dateJson ? JSON.parse(dateJson) : [today, nextday];

      const planList = await planModel.findAll({
        where: {
          date: {
            $in: dateList,
          },
      ...
      // 从查询到的生产计划列表中，筛选出化钢记录表中没有生产的炉号部分
      const list = planList.filter((plan) => {
        return meltList.every(
          (melt) => melt.furnace.indexOf(plan.furnace) <= -1
        );
      });
      res.send({
        status: 0,
  ...
  async updateData(req, res, next) {
    ...
    try {
      if (Number(roleId) === 1) {
        ...
        const [n] = await planModel.update(newData, {
          where: { planId },
        });
        // 一旦更改生产计划，则需要重新审批，状态变为“待审批”
        const [k] = await planModel.update(
          { approved: 0 },
          { where: { date, castId } }
        );
  ...
  async export(req, res, next) {
    ...
    try {
      ...
      conf.rows = list.map((item) => {
        ...
        qualifiedDemands.forEach((item) => {
          item.qualifiedRibbonToughnessLevels =
            item.qualifiedRibbonToughnessLevels.join();
          item.qualifiedAppearenceLevels =
            item.qualifiedAppearenceLevels.join();
        });
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=" + "shengchanjihua.xlsx"
      );
